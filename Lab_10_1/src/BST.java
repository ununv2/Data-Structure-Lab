...
public class BST {
	...
	private void inOrderTraversal(BSTNode node, List<Integer> sortedValues) {
	    if (node == null) return;
	    inOrderTraversal(node.left, sortedValues);
	    sortedValues.add(node.data); 
	    inOrderTraversal(node.right, sortedValues);
	}


	private BSTNode buildBalancedTree(List<Integer> sortedValues, int start, int end) {
	    if (start > end) return null;
	    
	    int mid = (start + end) / 2;
	    BSTNode node = new BSTNode(sortedValues.get(mid));
