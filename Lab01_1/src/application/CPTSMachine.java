...
public class CPTSMachine {
	...
	public static boolean addStation(String name) {
		if(!isStationExisted(name)) {
			stationlist.add(new Station(name,totalStationNumber+1));
	...
	public static boolean buyTicket(int type, Station start, Station end) {
		...
		if (start != null && end != null) {
			if(type>=0 && type <=2 && isStationExisted(start.getName()) && isStationExisted(end.getName()) && ticket.isStationValid(start,end)) {
				/* FILL CODE */
